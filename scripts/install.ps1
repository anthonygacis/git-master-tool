...
Write-Host "`ngitcompare installer`n" -ForegroundColor Cyan
...
try {
    $Release = Invoke-RestMethod -Uri $ApiUrl -Headers @{ "User-Agent" = "gitcompare-installer" }
} catch {
...
$InstallDir = "$env:LOCALAPPDATA\Programs\gitcompare"
...
Write-Host "`nDone! Run:  gitcompare develop master`n" -ForegroundColor Green
